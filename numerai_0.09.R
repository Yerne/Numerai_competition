...
# XGBoost
setwd("C:\\Users\\TEMP.Main.001\\Documents\\GitHub\\20170605-Numerai_train\\1. Original Data\\81")
...
# X1 <- X - xmean
for (i in 1:nrow(X)) X1['era', X1[i,1]] <- X['era', X[i,1]] - xmean['era', xmean[,1]] 

for (i in 1:nrow(X)) { X1[i, ] <- X - xmean[match(X$era, xmean$era)] }
...
library(xgboost)
classifier = xgboost(data = as.matrix(dataset), objective = "binary:logistic", label = dataset$target, nrounds = 10)
